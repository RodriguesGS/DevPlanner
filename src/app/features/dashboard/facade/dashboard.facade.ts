...
export class DashboardFacade {
    ...
    async load() { this.tasksSig.set(await this.repository.listAll()); }

    total = computed(() => this.tasks().length);
    
    doneThisWeek = computed(() =>
        this.tasks().filter(t => t.completedAt && inLastDays(t.completedAt!, 7)).length
    );

    overdue = computed(() =>
        this.tasks().filter(t => isOverdue(t)).length
    );
